...
class Siswa_Feature extends Controller
{
    ...
    public function edit()
    {
        ...
        if ($validator->passes()) {
            $old_data = Siswa::where('id', Auth::guard('student')->user()->id)->first();

            if (request()->input('name') == $old_data->name && request()->input('username') == $old_data->username && request()->input('email') == $old_data->email) {
                if (request()->input('password') != null) {
                    Siswa::where('id', Auth::guard('student')->user()->id)->update(['password' => bcrypt(request()->input('password'))]);
                ...
                if ($new_data_check != null) {
                    ...
                    $new_data = array(
                        ...
                        'action_by' => Auth::guard('student')->user()->id
                    );
                    if (request()->input('password') != null) {
                        $new_data['password'] = request()->input('password');
                    };

                    Siswa::where('id', Auth::guard('student')->user()->id)->update($new_data);
